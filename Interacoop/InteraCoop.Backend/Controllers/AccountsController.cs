...
namespace InteraCoop.Backend.Controllers
{
    ...
    public class AccountsController : ControllerBase
    {
        ...
        public AccountsController(IUsersUnitOfWork usersUnitOfWork, IConfiguration config, IFileStorage fileStorage, IMailHelper mailHelper)
        {
        ...
        public async Task<IActionResult> CreateUser([FromBody] UserDto model)
        {
            ...
            if (result.Succeeded)
            {
                await _usersUnitOfWork.AddUserToRolesAsync(user, user.UserType.ToString());//Valida
                var response = await SendConfirmationEmailAsync(user);
                if (response.WasSuccess)
                {
                    return NoContent();
